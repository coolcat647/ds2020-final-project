...
kitti_raw_train['cal'] = os.path.join(base_data, 'kitti', 'train', 'calib')
kitti_raw_train['ims'] = os.path.join(base_data, 'kitti', 'train', 'image_2')
kitti_raw_train['lab'] = os.path.join(base_data, 'kitti', 'train', 'label_2')
kitti_raw_train['pre'] = os.path.join(base_data, 'kitti', 'train', 'prev_2')

kitti_raw_val = dict()
kitti_raw_val['cal'] = os.path.join(base_data, 'kitti', 'val', 'calib')
kitti_raw_val['ims'] = os.path.join(base_data, 'kitti', 'val', 'image_2')
kitti_raw_val['lab'] = os.path.join(base_data, 'kitti', 'val', 'label_2')
kitti_raw_val['pre'] = os.path.join(base_data, 'kitti', 'val', 'prev_2')
...
# Rewrite train.txt
files_list = [raw_filename[:-4] for raw_filename in os.listdir(kitti_raw_train['cal']) if not raw_filename.lower().startswith(".")]
tra_file = os.path.join(base_data, split, 'train.txt')
text_file = open(tra_file, 'w+')
for idx in range(len(files_list)):
    text_file.write(files_list[idx] + '\n')
text_file.close()

# Rewrite val.txt
files_list = [raw_filename[:-4] for raw_filename in os.listdir(kitti_raw_val['cal']) if not raw_filename.lower().startswith(".")]
val_file = os.path.join(base_data, split, 'val.txt')
text_file = open(val_file, 'w+')
for idx in range(len(files_list)):
    text_file.write(files_list[idx] + '\n')
text_file.close()
...
for line in text_file:
    ...
    if parsed is not None:
        ...
        if not os.path.exists(os.path.join(kitti_val['cal'], str(new_id) + '.txt')):
            os.symlink(os.path.join(kitti_raw_val['cal'], str(id) + '.txt'), os.path.join(kitti_val['cal'], str(new_id) + '.txt'))

        if not os.path.exists(os.path.join(kitti_val['ims'], str(new_id) + '.png')):
            os.symlink(os.path.join(kitti_raw_val['ims'], str(id) + '.png'), os.path.join(kitti_val['ims'], str(new_id) + '.png'))

        if not os.path.exists(os.path.join(kitti_val['pre'], str(new_id) + '_01.png')):
            os.symlink(os.path.join(kitti_raw_val['pre'], str(id) + '_01.png'), os.path.join(kitti_val['pre'], str(new_id) + '_01.png'))
        
        if not os.path.exists(os.path.join(kitti_val['pre'], str(new_id) + '_02.png')):
            os.symlink(os.path.join(kitti_raw_val['pre'], str(id) + '_02.png'), os.path.join(kitti_val['pre'], str(new_id) + '_02.png'))
        
        if not os.path.exists(os.path.join(kitti_val['pre'], str(new_id) + '_03.png')):
            os.symlink(os.path.join(kitti_raw_val['pre'], str(id) + '_03.png'), os.path.join(kitti_val['pre'], str(new_id) + '_03.png'))

        if not os.path.exists(os.path.join(kitti_val['lab'], str(new_id) + '.txt')):
            os.symlink(os.path.join(kitti_raw_val['lab'], str(id) + '.txt'), os.path.join(kitti_val['lab'], str(new_id) + '.txt'))
